...
function S_AppliedJobs(prop) { // need  a function to close this popup
    const [Apjbs, setApjbs] = useState([]);
    async function fetchApjbs() {
        const sid = GetUserId('s_userId');
        const response = await axios.get(`http://localhost:3002/getapldjbs?sid=${sid}`)
    ...
    async function handleJobCancel(index){
        ...
        await axios.post("http://localhost:3002/applyjob",{pjid:pjid,sid:GetUserId("s_userId"),action:"cancel"});
        await fetchApjbs();
    }
