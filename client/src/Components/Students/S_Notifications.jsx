...
function NotificationCard({ title, status, invLink }) {
    ...
    return (
        <div className={`notification new ${status === "Rejected" ? "rejected" : ""}`}>
        ...
        <div className={`popup-container ${isPopupOpen ? 'active' : ''}`}>
            <div className="popup-content">
                ...
                {Notis.map((Noti, index) => {
                    ...
                    return (
                        <NotificationCard key={index} title={title} status={status} invLink={invLink} />
                    );
                })}

                <button className="btn btn-outline-danger" onClick={() => prop.toggle_noti()}>CLOSE</button>
            </div>
        </div>
    );
}
